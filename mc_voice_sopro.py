...
"""What Sopro will accept in a recording, and what to call itself when it says no.

The window is Sopro's own -- five to twenty seconds is what Sopro V2 documents
-- and it is a value here rather than a constant inside the decoder, because a
second engine that clones from a recording has a different window and neither
engine's rules belong in the other's module.
"""

# The decoder, the resampler and the WAV constants moved to
# :mod:`mc_voice_reference` when a third engine that clones from a recording
# arrived. They are re-bound here under the names this module has always used
# rather than left behind as a second copy, because the resampler's 102 dB of
# alias rejection is a *measured* number -- ``TestTheResampler`` sweeps a tone
# against it -- and a copy nobody swept would fail by making every clone sound
# slightly worse than its reference rather than by raising anything.
WAVE_PCM, WAVE_FLOAT, WAVE_EXTENSIBLE = (reference.WAVE_PCM, reference.WAVE_FLOAT,
                                         reference.WAVE_EXTENSIBLE)
ENCODING_NAMES = reference.ENCODING_NAMES
RESAMPLE_ZEROS = reference.RESAMPLE_ZEROS
RESAMPLE_ROLLOFF = reference.RESAMPLE_ROLLOFF
RESAMPLE_BETA = reference.RESAMPLE_BETA
RESAMPLE_BLOCK = reference.RESAMPLE_BLOCK
_encoding_label = reference.encoding_label
_clipped = reference.clipped
_decode_pcm16 = reference.decode_pcm16
_bessel_i0 = reference.bessel_i0
_kaiser_table = reference.kaiser_table
_resample = reference.resample
_filter_shape = reference.filter_shape
_resample_with = reference.resample_with
_resample_slowly = reference.resample_slowly


def normalize_reference(data: bytes) -> tuple:
    """A recording as canonical mono 24 kHz PCM16, or a refusal that says why.

    Section 25's checks, run by :func:`mc_voice_reference.normalize` against
    :data:`ENVELOPE` and refused in Sopro's own voice. What is Sopro's about it
    is the window and the name in the sentence; the decoding, the downmix and
    the resampling are the same arithmetic whichever engine asked for them.
    """
    return reference.normalize(data, ENVELOPE, SoproError)
