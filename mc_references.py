...
# --------------------------------------------------------------------------- #
# The live ImageStitch source, for anything that needs one
# --------------------------------------------------------------------------- #
#
# Two features now ask "is there an ImageStitch image behind this generation":
# Model Chain's Stage 2 Pass Through, and Klein Spatial Layout. They ask it at
# different moments -- one while a panel is being repainted, one on the thread a
# job is already running on -- and they must not answer it differently.
#
# So the answer lives here, beside the component id and the extractor, and both
# callers subscribe to it. §9 asks for exactly that and gives the reason: a
# second, independent DOM scraper is a second place for the elem_id suffix to be
# written down, and the first time ImageStitch renames its gallery only one of
# them would be fixed.


def is_stitch_gallery(elem_id) -> bool:
    """Whether ``elem_id`` is ImageStitch's Reference Image(s) gallery.

    Matched on the tail, for the reason :data:`STITCH_GALLERY_SUFFIX` documents.
    A function rather than an inline ``endswith`` at each call site so that the
    knowledge of *how* the match is made stays in the module that owns the
    suffix.
    """
    return str(elem_id or "").endswith(STITCH_GALLERY_SUFFIX)


class LiveSource:
    """What ImageStitch is holding right now, as the UI can see it.

    **Advisory only.** It is read from Gradio component values while a panel is
    being repainted, which is a snapshot of a browser rather than of a
    generation: the gallery can be emptied, the checkbox unticked or the whole
    script disabled between the repaint and the press. §9 and R4 both say the
    same thing about it -- the UI uses this to decide what to *offer*, and
    :func:`runtime_source` decides what actually happens.
    """

    __slots__ = ("installed", "enabled", "count", "known")

    def __init__(self, installed: bool, enabled: bool, count: int,
                 known: bool = True):
        self.installed = bool(installed)
        self.enabled = bool(enabled)
        self.count = max(int(count or 0), 0)
        self.known = bool(known)
        """Whether the gallery was actually read, or merely not handed over.

        Three states and not two, because the difference between them is a
        sentence that would otherwise be false. ImageStitch sorts after the
        panels that want to know about it, so a page can be built -- and a
        checkpoint can change -- at a moment when its gallery component exists
        for nobody to read. "Nothing was handed to me" is not "there are no
        images", and a panel that rendered the first as the second would tell
        somebody their gallery was empty while they were looking at three
        pictures in it.
        """

    @property
    def usable(self) -> bool:
        """Whether a mode needing an image should be offered.

        Permissive where the contents are unknown, and deliberately so. This
        decides what the panel *offers*, and the cost of the two mistakes is not
        symmetric: offering a mode that turns out to have no image behind it is
        a clear refusal at the next press, and withholding one that did have an
        image is a feature that looks broken with no way to find out why.
        """
        if not (self.installed and self.enabled):
            return False
        return bool(self.count) or not self.known

    def describe(self) -> str:
        """One short line naming what a mode needing an image would find.

        Written from the same facts the availability matrix is computed from, so
        a marked-unavailable radio button and the sentence under it cannot
        disagree about why.
        """
        if not self.installed:
            return "ImageStitch Integrated is not installed."
        if not self.enabled:
            return "ImageStitch is switched off."
        if not self.known:
            return ("ImageStitch is installed; what it holds is read when you "
                    "generate.")
        if not self.count:
            return "No active ImageStitch image."
        if self.count == 1:
            return "ImageStitch: 1 usable image."
        return (f"ImageStitch: {self.count} usable images; image 1 is primary where "
                f"required.")


def live_source(enabled=None, gallery=None) -> LiveSource:
    """The live ImageStitch state, from component values a handler was given.

    ``enabled`` and ``gallery`` are ImageStitch's own controls, passed in by
    whoever wired them as inputs. Both may be ``None``, which is not the same as
    False and empty -- see :attr:`LiveSource.known`.

    Deliberately no ``p``. Nothing about a live panel is a generation, and a
    function that could read either would eventually be asked which one it read.
    """
    installed = stitch_is_installed()
    if gallery is None and enabled is None:
        # Nothing was wired, or nothing was handed over. Assume the script's own
        # default -- on -- and say the contents are unknown rather than absent.
        return LiveSource(installed, installed, 0, known=False)
    return LiveSource(installed=installed,
                      enabled=True if enabled is None else bool(enabled),
                      count=len(extract_images(gallery)))


def runtime_source(p):
    """This generation's ImageStitch images: ``(enabled, images, max_dim)``.

    The authoritative answer, and the only one a sampling decision may be made
    from. It reads ``p.script_args`` through :func:`stitch_arguments`, which is
    the arguments the user's press actually carried, and resolves them through
    :func:`extract_images`, which is the same extractor Pass Through uses.

    ``enabled`` is False and ``images`` empty when ImageStitch is not installed
    at all, so a caller can treat "absent" and "switched off" alike without
    first asking which it was.

    What this deliberately does not consult, per §3 and §16: the img2img tab, the
    output gallery, a previous generation, a cached ``ref_latents`` list, a
    pasted PNG, a Stage 2 decoupled gallery, or ``reference_count(model)``. The
    last one is the dangerous one -- it reports what the *engine* is holding,
    which survives a job, so an emptied gallery would still look source-capable
    for exactly as long as it takes somebody to be surprised by it.
    """
    arguments = stitch_arguments(p)
    if arguments is None:
        return False, (), DEFAULT_MAX_DIM

    enabled, gallery, max_dim = arguments
    if not enabled:
        return False, (), max_dim
    return True, tuple(extract_images(gallery)), max_dim
